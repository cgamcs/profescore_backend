...
class DashboardController {
    static getRecentActivities = async (req, res) => {
        try {
            ...
            const formattedActivities = await Promise.all(recentActivities.map(async (activity) => {
                ...
                switch (activity.onModel) {
                    case 'Faculty':
                        const faculty = await Faculty_1.default.findById(activity.relatedEntity);
                        entityName = faculty ? faculty.name : 'Facultad eliminada';
                        break;
                    case 'Subject':
                        const subject = await Subject_1.default.findById(activity.relatedEntity);
                        entityName = subject ? subject.name : 'Materia eliminada';
                        break;
                    case 'Professor':
                        const professor = await Professor_1.default.findById(activity.relatedEntity);
    ...
    static getDashboardStats = async (req, res) => {
        try {
            const facultiesCount = await Faculty_1.default.countDocuments();
            const subjectsCount = await Subject_1.default.countDocuments();
            const professorsCount = await Professor_1.default.countDocuments();
            const ratingsCount = await Rating_1.default.countDocuments(); // Assuming you have this model
            res.json({
                facultiesCount,
                subjectsCount,
                professorsCount,
